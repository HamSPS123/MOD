<?php
    ...
    if (isset($_POST['d_id']) != "") {
        ...
        WHERE d_id='$d_id' ORDER BY level_name ASC,course_id ASC;");
...
<div class="table-responsive2">
    ...
    <table id="detail_table" class="table-bordered" style="width: 1200px;">
        ...
        WHERE qd.d_id='$d_id' AND p_stt=1 ORDER BY qd.p_m_id ASC;");
        if (mysqli_num_rows($result_quota) > 0) {
            foreach($result_quota as $row){
                $amount += ($row["std_qty"] + $row["emp_qty"]);
                $i++;
        ...
        <?php
                    ...
                    echo '  <tr class="collapse multi-collapse-province">
                                <td colspan="6" class="text-center text-danger"><p class="h6 m2">ບໍ່ມີຂໍ້ມູນການແຈກຢາຍ</p></td>
                            <tr>';
                }
                ?>
        <tr>
        ...
        LEFT JOIN `user` u ON qd.user_id=u.user_id
        LEFT JOIN hrd_user hu ON u.user_id=hu.user_id
        WHERE qd.d_id='$d_id' AND p_stt=2 ORDER BY qd.p_m_id ASC;");
        if (mysqli_num_rows($result_quota) > 0) {
            foreach($result_quota as $row){
                $amount += ($row["std_qty"] + $row["emp_qty"]);
                $i++;
...
$('.inputQuota').keyup(function() {
    ...
    <?php
            ...
            LEFT JOIN `user` u ON qd.user_id=u.user_id
            LEFT JOIN hrd_user hu ON u.user_id=hu.user_id
            WHERE qd.d_id='$d_id' ORDER BY qd.p_m_id ASC");
