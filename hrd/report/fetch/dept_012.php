...
<div class="table-responsive tscroll">
    <table class="table table-bordered text-center">
        ...
        (univer+prov+ad_test+std+emp) as total,
        ...
        foreach($sql as $row){
            ...
            univer+prov+ad_test+std+emp as total,
            univer,prov,ad_test,std,emp,user_id
            FROM statistic_detail sd
            LEFT JOIN statatic s ON sd.st_id=s.st_id
            LEFT JOIN statatic_group sg ON s.st_group=sg.group_id
        ...
        sum(univer) + sum(univer_valunteer) + sum(prov) + sum(intv) + sum(ad_test) + sum(std) + sum(emp) + sum(std_handi) + sum(std_lan)+sum(ssk)+sum(std)+sum(emp_gov)+sum(test)+sum(std_con)+sum(std_2lan)+sum(std_inter)+sum(emp_private)
        ...
        if(mysqli_num_rows($sql)){
            ...
            foreach($sql2 as $key2 => $row2){
                ...
                $sql3 = mysqli_query($conn, "SELECT d_id,u.uni_id,uni_name,f.fac_id,fac_name,dept_name,ad.course_id,course_name,get_plan,univer,univer_valunteer,prov,intv,ad_test,std,emp,std_handi,std_lan,ssk,std,emp_gov,test,std_con,std_2lan,std_inter,emp_private,
                univer+univer_valunteer+prov+intv+ad_test+std+emp+std_handi+std_lan+ssk+std+emp_gov+test+std_con+std_2lan+std_inter+emp_private as total,level_name
