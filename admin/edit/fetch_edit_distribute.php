...
include ($path."oop/connect.php");
  if (isset($_POST['query']) != "") {
    ...
    $dept_id = $fac_row['dept_id'];
        ...
        sum(emp) as emp,sum(std_handi) as std_handi,
        ...
        WHERE ad.adj_id='$adj_id' AND d.dept_id='$dept_id' GROUP BY u.uni_id ORDER BY uni_name ASC;");
...
<div class="table-responsive tscroll">
    ...
    <table id="detail_table" class="table-bordered" style="width: 3100px;">
        ...
        <?php
            ...
            sum(univer)+sum(prov)+sum(intv)+sum(ad_test)+sum(emp)+sum(std_handi)+sum(std_lan)+sum(ssk)+sum(std)+sum(emp_gov)+sum(test)+sum(std_con)+sum(std_2lan)+sum(std_inter)+sum(emp_private) as total_fac
            FROM adjustdetail ad
            LEFT JOIN course c ON ad.course_id=c.course_id
            LEFT JOIN faculty f ON c.fac_id=f.fac_id
            LEFT JOIN university u ON f.uni_id=u.uni_id
            LEFT JOIN department d ON u.dept_id=d.dept_id
            LEFT JOIN adjust a ON ad.adj_id=a.adj_id
            WHERE ad.adj_id='$adj_id' AND d.dept_id='$dept_id' AND f.uni_id='$uni_ids' GROUP BY f.fac_id ORDER BY fac_name ASC;");
            $i=0;
        ...
        <?php
            //Loop Course
            $result_course = mysqli_query($conn, "SELECT d_id,u.uni_id,uni_name,f.fac_id,fac_name,dept_name,ad.course_id,course_name,univer,prov,intv,ad_test,emp,std_handi,std_lan,ssk,std,emp_gov,test,std_con,std_2lan,std_inter,emp_private,
            ...
            WHERE ad.adj_id='$adj_id' AND d.dept_id='$dept_id' AND f.uni_id='$uni_ids' AND c.fac_id='$fac_id' ORDER BY level_name ASC,course_name ASC;");
            $j=0;
        ...
        <tr class="child-row1" style="display: table-row;">
            ...
            <td class="text-nowrap">
                <div class="d-flex py-2 px-2" style="gap: 25px;">
