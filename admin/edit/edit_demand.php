...
<form action="edit-demand" method="Post">
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-8 input-group mb-3">
            <br>
            <select name="status" id="status" style="width: 250px;text-align-last: center;" class="custom-select form-control">
                <option value="" disabled selected>ເລືອກກົມ</option>
                <?php
                    $fy = mysqli_query($conn, "SELECT adj_id,ad.fy_id,ad.dept_id,dept_name FROM adjust ad left JOIN fy ON ad.fy_id=fy.fy_id left JOIN department d ON ad.dept_id=d.dept_id");
            ...
            <select name="p_m_id_select" id="p_m_id_select" style="width: 250px;text-align-last: center;" class="custom-select form-control">
            <option value="" disabled selected>ເລືອກແຂວງ</option>
                <?php
                    $obj->select_province();                    
                    if (mysqli_num_rows($result) > 0) {
                    while ($row_prov = mysqli_fetch_array($result)) {
                        ?>
                <option value="<?= $row_prov['p_m_id'] ?>"><?= $row_prov['p_m_name'] ?></option>
                <?php
                    }
                }else{
                    echo '<option disabled>ສູນກາງຍັງບໍ່ເປີດສົກຮຽນ</option>';
...
<?php
    ...
    if (isset($_POST['btnConfirm'])) {
        ...
        $adj_id = $_POST['adj_id'];
        $p_m_id_select = $_POST['p_m_id_select'];
        $item = array();
        ...
        WHERE adj_id='$adj_id' AND cp.p_m_id='$p_m_id_select'
        GROUP BY f.uni_id;");
        foreach($university as $uni){
            ...
            LEFT JOIN course c ON cp.course_id=c.course_id
            LEFT JOIN faculty f ON c.fac_id=f.fac_id
            WHERE adj_id='$adj_id' AND uni_id='$uni_id' AND cp.p_m_id='$p_m_id_select'
            GROUP BY c.fac_id;");
            foreach ($faculty as $fac) {
                $fac_id = $fac['fac_id'];

                $course = mysqli_query($conn,"SELECT cp.user_id,cre_id,demand_name,surname,gender,adj_id,cp.course_id,c.fac_id,course_name,cp.p_m_id,std_qty as std_qty,emp_qty as emp_qty, std_qty + emp_qty as total
                FROM create_plan cp
                LEFT JOIN course c ON cp.course_id=c.course_id
                LEFT JOIN user u ON cp.user_id=u.user_id
                LEFT JOIN demand_user du ON u.user_id=du.user_id
                WHERE adj_id='$adj_id' AND c.fac_id='$fac_id' AND cp.p_m_id='$p_m_id_select';");
                    foreach ($course as $cour) {
                        $user_ids = $cour["user_id"];
                        $std = "std_".$cour["course_id"]."_".$cour["user_id"];
                        $emp = "emp_".$cour["course_id"]."_".$cour["user_id"];
                        $cre_id = "cre_id_".$cour['cre_id']."_".$cour['user_id'];
                        $course_id = $cour['course_id'];

                        $item[] = ["cre_id" => $_POST["$cre_id"],"std_qty" => $_POST["$std"],"emp_qty" => $_POST["$emp"]];
                    }
            }
        }
...
$(document).ready(function() {
    ...
    function load_data(query,p_m_id) {
        $.ajax({
            url: "fetch_edit_demand.php",
            method: "POST",
            data: {
                query: query,
                p_m_id: p_m_id
    ...
    $('#status').change(function() {
        var page = "0";
        $("#amount").val(0);
        var p_m_id_select = $("#p_m_id_select").val();
        var status_id = $(this).val();
        if (status_id != '') {
            load_data(status_id,p_m_id_select);
        } else {
            load_data('%%',p_m_id_select);
        }
    });
    $('#p_m_id_select').change(function() {
        var page = "0";
        $("#amount").val(0);
        var status_id = $("#status").val();
        var p_m_id_select = $("#p_m_id_select").val();
        console.log(p_m_id_select);
        if (p_m_id_select != '') {
            load_data(status_id,p_m_id_select);
        } else {
            load_data(status_id,"%%");
