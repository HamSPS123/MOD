...
<div class="table-responsive tscroll">
    ...
    <table id="detail_table" class="table-bordered" style="width: 1500px;">
        ...
        <tr class="child-<?php echo $fac['fac_id'] ?> btnUpdate_demand" style="display: table-row;">
            ...
            <td style="text-align:center">
                <input class="quantity" min="0" name="num1" id="total_course_<?php echo $course_id;?>"
            ...
            <td style="text-align:center;">
                <div class="def-number-input number-input safari_only mx-auto">
                    ...
                    <input class="quantity demandInput" min="0" name="std_<?php echo $cour['course_id'] ?>"
                        id="std_<?php echo $cour['course_id'] ?>" value="<?= $cour['std_qty'] ?>" type="number"
            ...
            <td style="text-align:center;">
                <div class="def-number-input number-input safari_only mx-auto">
                    <button type="button" onclick="this.parentNode.querySelector('input[type=number]').stepDown()"
                        class="minus demandBtn"></button>
                    <input class="quantity demandInput" min="0" name="emp_<?php echo $cour['course_id'] ?>"
                        id="emp_<?php echo $cour['course_id'] ?>" value="<?= $cour['emp_qty'] ?>" type="number"
                        style="font-weight:bold!important;">
                    <button type="button" onclick="this.parentNode.querySelector('input[type=number]').stepUp()"
                        class="plus demandBtn"></button>
                </div>
...
$('.demandBtn').on('click', function() {
    ...
    LEFT JOIN faculty f ON c.fac_id=f.fac_id
    LEFT JOIN university u ON f.uni_id=u.uni_id
    WHERE adj_id='$a_id' AND p_m_id='$p_id' AND `user_id`='$_SESSION[user_id]'
    GROUP BY f.uni_id ORDER BY uni_id ASC;");
    foreach($u as $un){
        $u_id = $un['uni_id'];
        ...
        LEFT JOIN course c ON cp.course_id=c.course_id
        LEFT JOIN faculty f ON c.fac_id=f.fac_id
        WHERE adj_id='$a_id' AND p_m_id='$p_id' AND uni_id='$u_id' AND `user_id`='$_SESSION[user_id]'
        GROUP BY c.fac_id;");
        //Loop ຄະນະອອກມາໃຫ້ຄືນກັນກັບຕາຕະລາງຂ້າງເທິງ
        foreach($f as $fc){
            $f_id = $fc['fac_id'];
            $c = mysqli_query($conn,"SELECT cre_id,adj_id,cp.course_id,c.fac_id,course_name,p_m_id,sum(std_qty) as std_qty,sum(emp_qty) as emp_qty, sum(std_qty)+ sum(emp_qty) as total
            FROM create_plan cp
            LEFT JOIN course c ON cp.course_id=c.course_id
            LEFT JOIN faculty f ON c.fac_id=f.fac_id
            WHERE adj_id='$a_id' AND p_m_id='$p_id' AND uni_id='$u_id' AND c.fac_id='$f_id' AND `user_id`='$_SESSION[user_id]'
            ...
            foreach($c as $cr){
                ...
                $c_sql = mysqli_query($conn,"SELECT cre_id,demand_name,surname,gender,adj_id,cp.course_id,c.fac_id,course_name,cp.p_m_id,sum(std_qty) as std_qty,sum(emp_qty) as emp_qty, sum(std_qty)+ sum(emp_qty) as total,level_name
                FROM create_plan cp
                LEFT JOIN course c ON cp.course_id=c.course_id
                LEFT JOIN faculty f ON c.fac_id=f.fac_id
                LEFT JOIN user u ON cp.user_id=u.user_id
                LEFT JOIN demand_user du ON u.user_id=du.user_id
                LEFT JOIN course_level lv ON c.level_id=lv.level_id
                WHERE adj_id='$adj_id' AND cp.p_m_id='$p_m_id' AND uni_id='$u_id' AND c.fac_id='$f_id' AND cp.`user_id`='$_SESSION[user_id]'
                GROUP BY cp.course_id  ORDER BY level_name ASC;");
                //ດຶງຂໍ້ມູນສາຂາວິຊາທີ່ຢູ່ມີເງື່ອນໄຂຢູ່ໃນຂະນະທີ່ ຄິກອອກເພື່ອເອົາມາບວກກັນ
                ...
                LEFT JOIN course c ON cp.course_id=c.course_id
                LEFT JOIN faculty f ON c.fac_id=f.fac_id
                WHERE adj_id='$adj_id' AND p_m_id='$p_m_id' AND uni_id='$u_id' AND `user_id`='$_SESSION[user_id]'
                GROUP BY c.fac_id ORDER BY fac_id ASC;");
    ...
    $uni_total = mysqli_query($conn,"SELECT cre_id,adj_id,cp.course_id,course_name,fac_name,f.uni_id,uni_name,p_m_id,sum(std_qty) as std_qty,sum(emp_qty) as emp_qty,
    sum(std_qty) + sum(emp_qty) as total
    FROM create_plan cp
    LEFT JOIN course c ON cp.course_id=c.course_id
    LEFT JOIN faculty f ON c.fac_id=f.fac_id
    LEFT JOIN university u ON f.uni_id=u.uni_id
    WHERE adj_id='$a_id' AND p_m_id='$p_id' AND `user_id`='$_SESSION[user_id]'
    GROUP BY f.uni_id ORDER BY uni_id ASC;"); //ດຶງຂໍ້ມູນມະຫາໄລທັງໝົດອອກມາ
    ?>
