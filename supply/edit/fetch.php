...
session_start();
 ...
 if($_POST["query"]){
    ...
    $row_fy2 = mysqli_fetch_array($fy2,MYSQLI_ASSOC);
     ...
     $no_ = 0;
     $unitversity = mysqli_query($conn,"SELECT u.uni_id,uni_name,sum(get_plan) AS total_uni FROM adjustdetail a LEFT JOIN course c ON a.course_id=c.course_id LEFT JOIN faculty f ON c.fac_id=f.fac_id LEFT JOIN university u ON f.uni_id=u.uni_id WHERE u.uni_id='$uni_id' AND adj_id='$adj_id' GROUP BY u.uni_id ORDER BY uni_id ASC;");
...
$('.demandBtn').on('click', function() {
    ...
    $u = mysqli_query($conn,"SELECT u.uni_id,uni_name,sum(get_plan) AS total_uni FROM adjustdetail a LEFT JOIN course c ON a.course_id=c.course_id LEFT JOIN faculty f ON c.fac_id=f.fac_id LEFT JOIN university u ON f.uni_id=u.uni_id WHERE u.uni_id='$uni_id' AND adj_id='$adj_id' GROUP BY u.uni_id;");
    foreach($u as $un){
        $u_id = $un['uni_id'];
