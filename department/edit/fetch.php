...
session_start();
 ...
 if($_POST["query"]){
    ...
    // $dpt = mysqli_query($conn, "SELECT dept_id from university WHERE uni_id='$uni_id'");
...
<div class="table-responsive tscroll">
    <table id="detail_table" class="table-bordered" style="width: 1200px;">
        ...
        <?php
                $unit_id = $uni["uni_id"];
                $i=0;
                $faculty = mysqli_query($conn,"SELECT f.fac_id,fac_name,sum(get_plan) AS total_fac FROM adjustdetail a LEFT JOIN course c ON a.course_id=c.course_id LEFT JOIN faculty f ON c.fac_id=f.fac_id LEFT JOIN university u ON f.uni_id=u.uni_id WHERE u.uni_id='$unit_id' AND adj_id='$adj_id' GROUP BY f.fac_id ORDER BY f.fac_id ASC;");
        ...
        <?php
                $j=0;
                $course = mysqli_query($conn,"SELECT d_id,a.course_id,course_name,get_plan,sup_name,su.surname,concat(hrd_name,'(Admin)') as admins,concat(usr_name,'(Department)') as dept_user,level_name FROM adjustdetail a LEFT JOIN adjust ad ON a.adj_id=ad.adj_id LEFT JOIN course c ON a.course_id=c.course_id LEFT JOIN faculty f ON c.fac_id=f.fac_id LEFT JOIN university u ON f.uni_id=u.uni_id LEFT JOIN supply_user su ON a.user_id=su.user_id LEFT JOIN hrd_user hs ON a.user_id=hs.user_id LEFT JOIN department_user ds ON a.user_id=ds.user_id  LEFT JOIN course_level lv ON c.level_id=lv.level_id WHERE f.fac_id='$fac_id' AND ad.fy_id='$fy_id' GROUP BY a.course_id ORDER BY level_name ASC,c.course_id ASC;");
        ...
        <tr class="child-<?php echo $fac_id ?>" style="display: table-row;">
            ...
            <td style="text-align:center;font-size: 14px!important;">
                <?php echo $cour['level_name'] ?><input type="hidden" name="d_id_<?= $cour["d_id"] ?>" value="<?php echo $cour["d_id"]?>">
            </td>
            <td>
...
$('.demandBtn').on('click', function() {
    ...
    $u = mysqli_query($conn,"SELECT u.uni_id,uni_name,sum(get_plan) AS total_uni FROM adjustdetail a LEFT JOIN course c ON a.course_id=c.course_id LEFT JOIN faculty f ON c.fac_id=f.fac_id LEFT JOIN university u ON f.uni_id=u.uni_id WHERE u.dept_id='$dept_id' AND adj_id='$adj_id' GROUP BY u.uni_id;");
    foreach($u as $un){
        $u_id = $un['uni_id'];
        $f = mysqli_query($conn,"SELECT f.fac_id,fac_name,sum(get_plan) AS total_fac FROM adjustdetail a LEFT JOIN course c ON a.course_id=c.course_id LEFT JOIN faculty f ON c.fac_id=f.fac_id LEFT JOIN university u ON f.uni_id=u.uni_id WHERE u.uni_id='$u_id' AND adj_id='$adj_id' GROUP BY f.fac_id;");
        //Loop ຄະນະອອກມາໃຫ້ຄືນກັນກັບຕາຕະລາງຂ້າງເທິງ
        foreach($f as $fc){
            $f_id = $fc['fac_id'];
            $c = mysqli_query($conn,"SELECT a.course_id,course_name,get_plan,sup_name FROM adjustdetail a LEFT JOIN course c ON a.course_id=c.course_id LEFT JOIN faculty f ON c.fac_id=f.fac_id LEFT JOIN university u ON f.uni_id=u.uni_id LEFT JOIN supply_user su ON a.user_id=su.user_id WHERE f.fac_id='$f_id' AND adj_id='$adj_id' GROUP BY a.course_id;");
            ...
            foreach($c as $cr){
                ...
                //ດຶງຂໍ້ມູນສາຂາວິຊາທີ່ຢູ່ມີເງື່ອນໄຂຢູ່ໃນຂະນະທີ່ ຄິກອອກເພື່ອເອົາມາບວກກັນ
                $f_sql = mysqli_query($conn,"SELECT f.fac_id,fac_name,sum(get_plan) AS total_fac FROM adjustdetail a LEFT JOIN course c ON a.course_id=c.course_id LEFT JOIN faculty f ON c.fac_id=f.fac_id LEFT JOIN university u ON f.uni_id=u.uni_id WHERE u.uni_id='$u_id' AND adj_id='$adj_id' GROUP BY f.fac_id;");
    ...
    <?php
            $f_sql = mysqli_query($conn,"SELECT f.fac_id,fac_name,sum(get_plan) AS total_fac FROM adjustdetail a LEFT JOIN course c ON a.course_id=c.course_id LEFT JOIN faculty f ON c.fac_id=f.fac_id LEFT JOIN university u ON f.uni_id=u.uni_id WHERE u.uni_id='$u_id' AND adj_id='$adj_id' GROUP BY f.fac_id;");//ດຶງຂໍ້ມູນຄະນະພາຍໃນວິທະຍາໄລດຽວ
    ...
    <?php
        ...
        ?>
    <?php
         $unit_total = mysqli_query($conn,"SELECT u.uni_id,uni_name,sum(get_plan) AS total_uni FROM adjustdetail a LEFT JOIN course c ON a.course_id=c.course_id LEFT JOIN faculty f ON c.fac_id=f.fac_id LEFT JOIN university u ON f.uni_id=u.uni_id WHERE u.dept_id='$dept_id' AND adj_id='$adj_id' GROUP BY u.uni_id;");
    }//ຈົບ Loop ມະຫາໄລ
    ...
    <?php
        ...
        ?>
    total_unit += parseInt($("#total_col_unit_get_plan_<?php echo $unit_get['uni_id'] ?>")
